...
DELIMITER //
CREATE OR REPLACE PROCEDURE GENREIFY()
BEGIN
  ...
  DECLARE EXIT HANDLER FOR 1062 /* dublicate */ ROLLBACK;
...
*/
DELIMITER //
CREATE OR REPLACE PROCEDURE NEW_BOOKS()
BEGIN
  START TRANSACTION;
...
/*
  ...
  на успешное завершение в случае возникновения ситуации взаимной блокировки с другими транзакциями.
*/
...
/*
  ...
  таким образом, чтобы она возвращала максимально корректные данные, даже если для достижения
  этого результата придётся пожертвовать производительностью.
*/
DELIMITER //

CREATE OR REPLACE PROCEDURE COUT(table_name TEXT)
BEGIN
  SET TRANSACTION
